...
He will check if the TCP port is used by a darkComet operator. He will check if he use the default password.
]]

---
-- @output
-- 21/tcp open  ftp
-- |_ darkCometScript: DarkComet 5.1 use default password, Banner: BF7CAB464EFB
-- @arg darkCometScript.ports Which ports to grab. Same syntax as -p option. Use
--                   	"common" to only grab common text-protocol banners.
--                   	Default: all ports.
-- @arg darkCometScript.timeout How long to wait for a banner. Default: 5s
-- @arg darkCometScript.idtype Give an specific idtype to check. The value is an  hex value from length 12


author = "jah & Rosalie Truong"
...
action = function( host, port )
  ...
  --check if the banner has a darkComet pattern
  if out and out:match("^%x%x%x%x%x%x%x%x%x%x%x%x$") then -- %x : hex value --> idtype has a length of 12
	
	--Check if an idtype is given in parameter and show if it match to the banner
	local idtype = stdnse.get_script_args(SCRIPT_NAME .. ".idtype")	
    if idtype and out == idtype then
		out = out .. " match with idtype in parameter"
	end
	
	--check if it is the default password from a darkComet version
	if out == "BF7CAB464EFB" then
		out = "DarkComet 5.1 use default password, Banner: " .. out
	elseif out == "1164805C82EE" then
		out = "DarkComet 5 use default password, Banner: " .. out
	elseif out == "155CAD31A61F" then
		out = "DarkComet 4.2F use default password, Banner: " .. out
	elseif out == "C7CF9C7CD932" then
		out = "DarkComet 4.2 use default password, Banner: " .. out
	elseif out == "B47CB892B702" then
		out = "DarkComet 4 use default password, Banner: " .. out
	elseif out == "8EA4AB05FA7E" then
		out = "DarkComet 2 use default password, Banner: " .. out
	else
		out = "DarkComet probably, Banner: " .. out
	end
